    ...
    <header class="flex justify-between shadow-sm items-center p-4 w-full fixed z-10 top-0 left-0 bg-white">
        ...
        <div class="block md:hidden z-40">
            <Icon
                name="tabler:menu"
                :key="menuOpen"
                :class="['w-6 h-6 text-black cursor-pointer', menuOpen ? 'hidden' : 'block']"
                @click="menuOpen = true"
            />
            <Icon
                name="tabler:x"
                @click="menuOpen = false"
                :class="['cursor-pointer z-40 top-8 right-8 text-black w-8 h-8', menuOpen ? 'block' : 'hidden']"
            />
        </div>
        <div :class="['fixed top-0 left-0 z-30', menuOpen ? 'block md:hidden' : 'hidden']">
